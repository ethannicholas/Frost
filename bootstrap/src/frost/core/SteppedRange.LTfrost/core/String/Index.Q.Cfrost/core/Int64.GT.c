#include "frost/core/SteppedRange.LTfrost/core/String/Index.Q.Cfrost/core/Int64.GT.h"
#include "frost/core/Value.h"
#include "frost/core/Class.h"
#include "frost/core/String.h"
#include "frost/core/Object.h"
#include "frost/core/String/Index.h"
#include "frost/core/Int64.h"
#include "frost/core/Bit.h"
#include "frost/core/MutableString.h"
#include "frost/core/Frost.h"

__attribute__((weak)) frost$core$String* frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$convert$R$frost$core$String$shim(frost$core$Object* p0) {
    frost$core$String* result = frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$convert$R$frost$core$String(((frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$wrapper*) p0)->value);

    return result;
}
__attribute__((weak)) void frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$cleanup$shim(frost$core$Object* p0) {
    frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$cleanup(((frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$wrapper*) p0)->value);

}

static frost$core$String $s1;
frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$class_type frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$class = { (frost$core$Class*) &frost$core$Class$class, -999, &$s1, (frost$core$Class*) &frost$core$Value$class, NULL, { frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$convert$R$frost$core$String$shim, frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$cleanup$shim} };

static frost$core$String $s2;
frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$wrapperclass_type frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$wrapperclass = { (frost$core$Class*) &frost$core$Class$class, -999, &$s2, (frost$core$Class*) &frost$core$Value$class, NULL, { frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$convert$R$frost$core$String$shim, frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$cleanup$shim} };
static frost$core$String $s1 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "", 67, -4100451375981612613, NULL };
static frost$core$String $s2 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "", 67, -4100451375981612613, NULL };
static frost$core$String $s18 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "\x53\x74\x65\x70\x70\x65\x64\x52\x61\x6e\x67\x65\x2e\x66\x72\x6f\x73\x74", 18, -6556486771978878285, NULL };
static frost$core$String $s19 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "", 79, 5179845231507618349, NULL };
static frost$core$String $s25 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "\x2e\x2e\x2e", 3, 1504239, NULL };
static frost$core$String $s27 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "\x2e\x2e", 2, 14893, NULL };
static frost$core$String $s36 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "\x53\x74\x65\x70\x70\x65\x64\x52\x61\x6e\x67\x65\x2e\x66\x72\x6f\x73\x74", 18, -6556486771978878285, NULL };
static frost$core$String $s37 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "", 79, 5179845231507618349, NULL };
static frost$core$String $s41 = { (frost$core$Class*) &frost$core$String$class, -999, (frost$core$Char8*) "\x20\x62\x79\x20", 4, 138041984, NULL };

frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$init$frost$core$String$Index$Q$frost$core$String$Index$Q$frost$core$Int64$frost$core$Bit(frost$core$String$Index$nullable param0, frost$core$String$Index$nullable param1, frost$core$Int64 param2, frost$core$Bit param3) {

frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT local0;
// line 43
frost$core$String$Index$nullable* $tmp3 = &(&local0)->start;
*$tmp3 = param0;
// line 44
frost$core$String$Index$nullable* $tmp4 = &(&local0)->end;
*$tmp4 = param1;
// line 45
frost$core$Int64* $tmp5 = &(&local0)->step;
*$tmp5 = param2;
// line 46
frost$core$Bit* $tmp6 = &(&local0)->inclusive;
*$tmp6 = param3;
frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT $tmp7 = *(&local0);
return $tmp7;

}
frost$core$String* frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$convert$R$frost$core$String(frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT param0) {

frost$core$MutableString* local0 = NULL;
// line 51
frost$core$MutableString* $tmp8 = (frost$core$MutableString*) frostObjectAlloc(48, (frost$core$Class*) &frost$core$MutableString$class);
frost$core$MutableString$init($tmp8);
*(&local0) = ((frost$core$MutableString*) NULL);
frost$core$Frost$ref$frost$core$Object$Q(((frost$core$Object*) $tmp8));
frost$core$MutableString* $tmp9 = *(&local0);
frost$core$Frost$unref$frost$core$Object$Q(((frost$core$Object*) $tmp9));
*(&local0) = $tmp8;
frost$core$Frost$unref$frost$core$Object$Q(((frost$core$Object*) $tmp8));
// unreffing REF($1:frost.core.MutableString)
// line 52
frost$core$String$Index$nullable $tmp10 = param0.start;
frost$core$Bit $tmp11 = frost$core$Bit$init$builtin_bit($tmp10.nonnull);
bool $tmp12 = $tmp11.value;
if ($tmp12) goto block1; else goto block2;
block1:;
// line 53
frost$core$MutableString* $tmp13 = *(&local0);
frost$core$String$Index$nullable $tmp14 = param0.start;
frost$core$Bit $tmp15 = frost$core$Bit$init$builtin_bit($tmp14.nonnull);
bool $tmp16 = $tmp15.value;
if ($tmp16) goto block3; else goto block4;
block4:;
frost$core$Int64 $tmp17 = (frost$core$Int64) {53};
frost$core$Frost$assertionFailure$frost$core$String$frost$core$Int64$frost$core$String(&$s18, $tmp17, &$s19);
abort(); // unreachable
block3:;
frost$core$Object* $tmp20;
if ($tmp14.nonnull) {
    frost$core$String$Index$wrapper* $tmp21;
    $tmp21 = (frost$core$String$Index$wrapper*) frostObjectAlloc(24, (frost$core$Class*) &frost$core$String$Index$wrapperclass);
    $tmp21->value = ((frost$core$String$Index) $tmp14.value);
    $tmp20 = ((frost$core$Object*) $tmp21);
}
else {
    $tmp20 = NULL;
}
frost$core$MutableString$append$frost$core$Object($tmp13, $tmp20);
frost$core$Frost$unref$frost$core$Object$Q($tmp20);
// unreffing REF($29:frost.core.Object)
goto block2;
block2:;
// line 55
frost$core$Bit $tmp22 = param0.inclusive;
bool $tmp23 = $tmp22.value;
if ($tmp23) goto block5; else goto block7;
block5:;
// line 56
frost$core$MutableString* $tmp24 = *(&local0);
frost$core$MutableString$append$frost$core$String($tmp24, &$s25);
goto block6;
block7:;
// line 1
// line 59
frost$core$MutableString* $tmp26 = *(&local0);
frost$core$MutableString$append$frost$core$String($tmp26, &$s27);
goto block6;
block6:;
// line 61
frost$core$String$Index$nullable $tmp28 = param0.end;
frost$core$Bit $tmp29 = frost$core$Bit$init$builtin_bit($tmp28.nonnull);
bool $tmp30 = $tmp29.value;
if ($tmp30) goto block8; else goto block9;
block8:;
// line 62
frost$core$MutableString* $tmp31 = *(&local0);
frost$core$String$Index$nullable $tmp32 = param0.end;
frost$core$Bit $tmp33 = frost$core$Bit$init$builtin_bit($tmp32.nonnull);
bool $tmp34 = $tmp33.value;
if ($tmp34) goto block10; else goto block11;
block11:;
frost$core$Int64 $tmp35 = (frost$core$Int64) {62};
frost$core$Frost$assertionFailure$frost$core$String$frost$core$Int64$frost$core$String(&$s36, $tmp35, &$s37);
abort(); // unreachable
block10:;
frost$core$Object* $tmp38;
if ($tmp32.nonnull) {
    frost$core$String$Index$wrapper* $tmp39;
    $tmp39 = (frost$core$String$Index$wrapper*) frostObjectAlloc(24, (frost$core$Class*) &frost$core$String$Index$wrapperclass);
    $tmp39->value = ((frost$core$String$Index) $tmp32.value);
    $tmp38 = ((frost$core$Object*) $tmp39);
}
else {
    $tmp38 = NULL;
}
frost$core$MutableString$append$frost$core$Object($tmp31, $tmp38);
frost$core$Frost$unref$frost$core$Object$Q($tmp38);
// unreffing REF($64:frost.core.Object)
goto block9;
block9:;
// line 64
frost$core$MutableString* $tmp40 = *(&local0);
frost$core$MutableString$append$frost$core$String($tmp40, &$s41);
// line 65
frost$core$MutableString* $tmp42 = *(&local0);
frost$core$Int64 $tmp43 = param0.step;
frost$core$Int64$wrapper* $tmp44;
$tmp44 = (frost$core$Int64$wrapper*) frostObjectAlloc(24, (frost$core$Class*) &frost$core$Int64$wrapperclass);
$tmp44->value = $tmp43;
frost$core$MutableString$append$frost$core$Object($tmp42, ((frost$core$Object*) $tmp44));
frost$core$Frost$unref$frost$core$Object$Q(((frost$core$Object*) $tmp44));
// unreffing REF($76:frost.core.Object)
// line 66
frost$core$MutableString* $tmp45 = *(&local0);
frost$core$String* $tmp46 = frost$core$MutableString$finish$R$frost$core$String($tmp45);
frost$core$Frost$ref$frost$core$Object$Q(((frost$core$Object*) $tmp46));
frost$core$Frost$unref$frost$core$Object$Q(((frost$core$Object*) $tmp46));
// unreffing REF($83:frost.core.String)
frost$core$MutableString* $tmp47 = *(&local0);
frost$core$Frost$unref$frost$core$Object$Q(((frost$core$Object*) $tmp47));
// unreffing result
*(&local0) = ((frost$core$MutableString*) NULL);
return $tmp46;

}
void frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT$cleanup(frost$core$SteppedRange$LTfrost$core$String$Index$Q$Cfrost$core$Int64$GT param0) {

return;

}

